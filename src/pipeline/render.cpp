 ...
 */
static VkResult createLogicalDevice(VkPhysicalDevice phyDev, const std::unordered_set<uint32_t> vecIndex, uint32_t count, VkDevice *out) {
...
static bool getDeviceQueues(utils::vulkanContext &ctx, utils::QueueInfo &info) {
    vkGetDeviceQueue(ctx.dev, ctx.indices.graphicsFamilyIndex.value(), 0, info.vecGraphicsQueue.data());
    vkGetDeviceQueue(ctx.dev, ctx.indices.presentFamilyIndex.value(), 0, info.vecPresentQueue.data());
    return true;
}
...
static void drawFrame(utils::vulkanContext &ctx) {
    vkWaitForFences(ctx.dev, 1, &ctx.fence, VK_TRUE, UINT64_MAX);
    vkResetFences(ctx.dev, 1, &ctx.fence);

    uint32_t imageIndex;
    vkAcquireNextImageKHR(ctx.dev, ctx.swapChain, UINT64_MAX, ctx.imageAvailableSemaphore, VK_NULL_HANDLE, &imageIndex);

    vkResetCommandBuffer(ctx.vecCommandBuffer[0], /*VkCommandBufferResetFlagBits*/ 0);
    ...
    submitInfo.pCommandBuffers = ctx.vecCommandBuffer.data();

    VkSemaphore signalSemaphores[] = {ctx.renderFinishedSemaphore};
    submitInfo.signalSemaphoreCount = 1;
    submitInfo.pSignalSemaphores = signalSemaphores;

    if (vkQueueSubmit(ctx.queueInfo.vecGraphicsQueue[0], 1, &submitInfo, ctx.fence) != VK_SUCCESS) {
    ...
    vkQueuePresentKHR(ctx.queueInfo.vecPresentQueue[0], &presentInfo);
}
...
int main(int argc, char **argv) {
    ...
    CHECK_VULKAN(createLogicalDevice(ctx.phyDev, ctx.indices.getIndices(), ctx.queueInfo.queueCount, &ctx.dev), "failed to create logical device!\n");
    CHECK_BOOL(getDeviceQueues(ctx, ctx.queueInfo), nullptr);
