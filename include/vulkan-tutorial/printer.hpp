...
namespace printer {
using namespace std;
static void layers(const vector<VkLayerProperties> &arr) {
    fprintf(stdout, "available layers count: %d\n", static_cast<uint32_t>(arr.size()));
...
static void physicalDevice(VkPhysicalDevice device) {
    ...
    fprintf(stdout,
            ...
            "\t\tdeviceID: %d\n"
            "\t\tdeviceType: %d\n",
            properties.deviceName, properties.apiVersion, properties.driverVersion, properties.vendorID, properties.deviceID, properties.deviceType);

    fprintf(stdout, "\tdevice features:\n");
    cout                                                                                                                     // a
